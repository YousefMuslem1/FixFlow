...
class SimpleImage {
	...
	public function resizeToFit($width, $height) {
		if ($this->width >= $this->height) //resmin yatay veya kare resimse
		{
			if ($width > $this->width) {
		...
		/*imagealphablending($new, false);
			        imagesavealpha($new, true);
		*/
		$white = imagecolorallocate($new, 250, 250, 250);
		imagefill($new, 0, 0, $white);
		imagecopyresampled($new, $this->image, $x, $y, 0, 0, $this->width, $this->height, $this->width, $this->height);
		$this->width = $width;
		$this->height = $height;
		$this->image = $new;
		return $this;
